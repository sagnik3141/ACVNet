...
class ACVNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        #     pred2 = disparity_regression(pred2, self.maxdisp)
            ...
            return [pred_attention, ac_volume]
...
class ACVSPC(nn.Module):
    def __init__(self, maxdisp, attn_weights_only, freeze_attn_weights):
        super(ACVSPC, self).__init__()
        self.maxdisp = maxdisp
        self.attn_weights_only = attn_weights_only
        self.freeze_attn_weights = freeze_attn_weights
        self.num_groups = 40
        ...
        self.acv_sh = ACVNet(self.maxdisp, self.attn_weights_only, self.freeze_attn_weights)
        self.acv_med = ACVNet(self.maxdisp, self.attn_weights_only, self.freeze_attn_weights)
        self.acv_lg = ACVNet(self.maxdisp, self.attn_weights_only, self.freeze_attn_weights)
    ...
    def forward(self, left_sh, right_sh, left_med, right_med, left_lg, right_lg):

        pred_attention_sh, ac_volume_sh = self.acv_sh(left_sh, right_sh)
        pred_attention_med, ac_volume_med = self.acv_sh(left_med, right_med)
        pred_attention_lg, ac_volume_lg = self.acv_sh(left_lg, right_lg)

        ac_volume = torch.stack([ac_volume_sh, ac_volume_med, ac_volume_lg], dim=3)
        ac_volume = torch.flatten(ac_volume, start_dim=2, end_dim=3)
        ...
        cost0 = F.upsample(cost0, [self.maxdisp, left_sh.size()[2], left_sh.size()[3]], mode='trilinear')
        cost0 = torch.squeeze(cost0, 1)
        pred0 = F.softmax(cost0, dim=1)
        pred0 = disparity_regression(pred0, self.maxdisp)    

        cost1 = F.upsample(cost1, [self.maxdisp, left_sh.size()[2], left_sh.size()[3]], mode='trilinear')
        cost1 = torch.squeeze(cost1, 1)
        pred1 = F.softmax(cost1, dim=1)
        pred1 = disparity_regression(pred1, self.maxdisp)    

        cost2 = F.upsample(cost2, [self.maxdisp, left_sh.size()[2], left_sh.size()[3]], mode='trilinear')
        cost2 = torch.squeeze(cost2, 1)
        pred2 = F.softmax(cost2, dim=1)
        pred2 = disparity_regression(pred2, self.maxdisp)

        return [pred_attention_sh, pred_attention_med, pred_attention_lg, pred0, pred1, pred2]