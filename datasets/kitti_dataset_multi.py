...
class KITTIDatasetSPC(Dataset):
    def __init__(self, datapath, list_filename, num_frames_sh, num_frames_med, num_frames_lg, training):
        self.datapath = datapath
        #self.datapath_12 = kitti12_datapath
        self.num_frames_sh = num_frames_sh
    ...
    def load_image(self, filename):
        ...
        img_sh = np.mean(img[128-self.num_frames_sh//2:129+self.num_frames_sh], axis=0)
        ...
        return np.stack([img_sh]*3, axis=2), np.stack([img_med]*3, axis=2), np.stack([img_lg]*3, axis=2)
    ...
    def __getitem__(self, index):
        ...
        # random crop
        left_img_sh = left_img_sh[y1:y1 + crop_h, x1:x1 + crop_w]
        right_img_sh = right_img_sh[y1:y1 + crop_h, x1:x1 + crop_w]
        ...
        left_img_sh = processed(left_img_sh)
        right_img_sh = processed(right_img_sh)
        ...
        return {"left_sh": left_img_sh,
                "right_sh": right_img_sh,
                "left_med": left_img_med,
                "right_med": right_img_med,
                "left_lg": left_img_lg,
                "right_lg": right_img_med,
                "disparity": disparity}
