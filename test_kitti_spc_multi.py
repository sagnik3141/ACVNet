...
parser.add_argument('--loadckpt', default='checkpoints/base1/checkpoint_000599.ckpt', help='load the weights from a specific checkpoint')
...
logger = SummaryWriter(os.path.join(args.logdir, exp_name))

# dataset, dataloader
StereoDataset = __datasets__[args.dataset]
train_dataset = StereoDataset(args.datapath, args.trainlist, args.num_frames_sh, args.num_frames_med, args.num_frames_lg, True)
test_dataset = StereoDataset(args.datapath, args.testlist, args.num_frames_sh, args.num_frames_med, args.num_frames_lg, True)
...
if args.resume:
    ...
    all_saved_ckpts = [fn for fn in os.listdir(os.path.join(args.logdir, exp_name)) if fn.endswith(".ckpt")]
    all_saved_ckpts = sorted(all_saved_ckpts, key=lambda x: int(x.split('_')[-1].split('.')[0]))
    # use the latest checkpoint file
    loadckpt = os.path.join(os.path.join(args.logdir, exp_name), all_saved_ckpts[-1])
...
def test_sample(sample, compute_metrics=True):
    ...
    imgL_sh = imgL_sh.cuda().float()
    imgR_sh = imgR_sh.cuda().float()
    imgL_med = imgL_med.cuda().float()
    imgR_med = imgR_med.cuda().float()
    imgL_lg = imgL_lg.cuda().float()
    imgR_lg = imgR_lg.cuda().float()
    disp_gt = disp_gt.cuda().float()
    #optimizer.zero_grad()
    disp_ests = model(imgL_sh, imgR_sh, imgL_med, imgR_med, imgL_lg, imgR_lg)
...
# # testing
    ...
    for batch_idx, sample in enumerate(TestImgLoader):    
        ...
        if do_summary:
            save_scalars(logger, 'test', scalar_outputs, 0)
            save_images(logger, 'test', image_outputs, 0)
